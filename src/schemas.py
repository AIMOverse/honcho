...
class Demographics(BaseModel):
    ...
    gender: str  # Gender of the user (e.g., male, female, non_binary, decline_to_state)
    age_range: str  # Age range of the user (e.g., <18, 18-25, 26-40, 41-60, 60+)


class HabitsPermissions(BaseModel):
    """Represents user habits and permissions for data collection."""
    bedtime: str  # User's bedtime habit (e.g., before_22:00, around_23:00, after_midnight, irregular)
    biometric_consent: bool  # Whether the user consents to biometric data collection (true/false)


class EmotionalState(BaseModel):
    """Represents the primary emotions of a user."""
    primary_emotions: list[str]  # List of user's recent primary emotions (e.g., anxiety, happiness, calmness)


class MBTIPersonality(BaseModel):
    """Represents the MBTI personality assessment of a user."""
    energy: str  # User's energy type (e.g., extrovert, introvert)
    information: str  # User's information processing style (e.g., sensing, intuitive)
    decision: str  # User's decision-making style (e.g., thinking, feeling)
    lifestyle: str  # User's lifestyle preference (e.g., judging, perceiving)


class StressLevel(BaseModel):
    """Represents the stress level of a user."""
    stress_level: str  # User's stress level (e.g., 0.0-0.3: Beginner Sanctuary, >0.9: Endgame Warfare)
...
class UserPersona(BaseModel):
    ...
    personality_assessment: MBTIPersonality  # User's MBTI personality assessment
    stress_assessment: StressLevel  # User's stress level
...
class PersonaResponse(BaseModel):
    ...
    processed_data: dict  # Processed data from the request